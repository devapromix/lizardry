...
if ($action == 'tavern') {
		...
		switch ($user['char_region']) {
			...
			case 4:
				...
				break;
			case 5:
				$user['title'] = 'Таверна "Одинокий Бродяга"';
				$user['description'] = 'Вы входите в большую каменную таверну. Уже много столетий стоит она на этом месте и кажется, будто построили ее еще во времена основания самого города. Внутри таверны просторно, несколько залов забиты разношерстной людской массой всевозможных рас.';
				break;				
			case 6:
				$user['title'] = 'Таверна "Дубовые Кружки"';
				$user['description'] = 'Вы входите в таверну. Здесь шумно и людно. Хозяин таверны - старый злой гном - то и дело, что успевает материть каждого на чем свет стоит и напоминать, что будет с ними, если кто-то что-то умыкнет. Ясное дело, его никто не слушает. Все столы заняты и любопытных глаз уйма. Можно даже сказать, что каждая пара глаз любопытна и каждого вновь вошедшего встречают хмурым негодованием. Лампы горят вполсилы, хозяин явно экономит масло.';
				break;				
			case 7:
			...
			case 9:
				...
				break;				
			case 10:
				$user['title'] = 'Таверна "Глаз Мантикоры"';
				$user['description'] = '';
				break;				
		}
	$user['frame'] = 'tavern';	
